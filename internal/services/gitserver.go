...
func (g *GitServerService) GetGitProviderToken(ctx context.Context, gitServerName string) (string, error) {
	...
	if err := g.k8sClinet.Get(ctx, client.ObjectKey{Name: gitServerName, Namespace: g.namespace}, gitServer); err != nil {
		return "", err
	}

	secret := &corev1.Secret{}
	if err := g.k8sClinet.Get(ctx, client.ObjectKey{Name: gitServer.Spec.NameSshKeySecret, Namespace: g.namespace}, secret); err != nil {
		return "", err
	}
